...
def predict(X):
    ...
    output = contrib.nd.MultiBoxDetection(cls_probs, bbox_preds, anchors, nms_threshold=0.3)
...
def display(img, output, threshold):
    ...
    for row in output:
        ...
        point_y = 240 - (480 // enlargenum) // 2
        point_x = 320 - (640 // enlargenum) // 2
        the_center_point = (int(point_x + (point1[0] + point2[0]) / (2 * enlargenum)),
                            int(point_y + (point1[1] + point2[1]) / (2 * enlargenum)))
...
while True:
    ...
    while True:  # and  timeToolong == 0:
        ...
        i = int(received_size // 1024 // 1024 * 100)
        view_bar(i, 100)
        # time.sleep(0.3)
    ...
    # img1 = cv2.imread('/home/dingchao/bibiballfiles/bulue_three.png')
    img1 = cv2.imread('needtorec.jpg')
    imgenlarge = np.zeros((480 // enlargenum, 640 // enlargenum, 3), np.uint8)
    point_y = 240 - (480 // enlargenum) // 2
    point_x = 320 - (640 // enlargenum) // 2
    imgenlarge[:, :, 0] = img1[point_y:point_y + 480 // enlargenum, point_x:point_x + 640 // enlargenum, 0]
    imgenlarge[:, :, 1] = img1[point_y:point_y + 480 // enlargenum, point_x:point_x + 640 // enlargenum, 1]
    imgenlarge[:, :, 2] = img1[point_y:point_y + 480 // enlargenum, point_x:point_x + 640 // enlargenum, 2]
    #     imgmulti2[:,:,0] = img1[145:337,193:449,0]
    #     imgmulti2[:,:,1] = img1[145:337,193:449,1]
    #     imgmulti2[:,:,2] = img1[145:337,193:449,2]
    imgenlarge = cv2.resize(imgenlarge, (640, 480))
    # net = TinySSD(num_classes=1)
    # ctx, net = d2l.try_gpu(), TinySSD(num_classes=1)
    cv2.imwrite("muti3.jpg", imgenlarge)
    img = image.imread('./muti3.jpg')
    # img = image.imread('needtorec.jpg')
